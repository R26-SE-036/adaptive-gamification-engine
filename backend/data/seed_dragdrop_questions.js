...
const QUESTIONS = [
    ...
    /* ══════════════════════════════════════════════════════════════════════
       Additional questions, authored to fill single-question slots.

       78 of the bank's 112 (concept, level, format) slots held exactly one
       question, so a student meeting the same slot twice was guaranteed a
       repeat. These bring the Drag & Drop slots to three.
       ══════════════════════════════════════════════════════════════════════ */

    dragDrop('q_bool_or_06', 'ALWAYS_TRUE_OR_CONDITION', 'boolean_logic', 'Beginner',
        ['boolean ready = started && !finished;',
         ...
         '}'],
        [1, 3, 0, 5, 2, 4],
        ['Both conditions must hold, so && rather than ||.',
         'The boolean is computed before it is tested.',
         'run() belongs to the true branch.'],
        'With || this would be ready whenever either half held, including when the work had already finished.'),

    dragDrop('q_bool_or_07', 'ALWAYS_TRUE_OR_CONDITION', 'boolean_logic', 'Intermediate',
        ['boolean loaded = data != null;',
         'boolean usable = loaded && data.size() > 0;',
         'if (usable) {',
         '    render(data);',
         '}'],
        [2, 0, 4, 3, 1],
        ['usable depends on loaded, so loaded must exist first.',
         '&& short-circuits, which is what stops size() being called on null.',
         'The render call is guarded by the second boolean.'],
        'Each guard protects the next: loaded makes the size() call safe, and usable makes render() safe.'),

    dragDrop('q_bool_or_08', 'ALWAYS_TRUE_OR_CONDITION', 'boolean_logic', 'Advanced',
        ['boolean inBounds = i >= 0 && i < arr.length;',
         'boolean nonZero = inBounds && arr[i] != 0;',
         'if (nonZero) {',
         '    total += arr[i];',
         '} else {',
         '    skipped++;',
         '}'],
        [4, 1, 6, 0, 3, 2, 5],
        ['The bounds check has to precede any read of arr[i].',
         'nonZero cannot be computed before inBounds exists.',
         'Both branches come after the condition they belong to.'],
        'Two chained short-circuits: inBounds makes arr[i] safe to read, and nonZero decides which branch runs.'),

    dragDrop('q_bool_or_09', 'ALWAYS_TRUE_OR_CONDITION', 'boolean_logic', 'Advanced',
        ['if (token == null) {',
         '    return false;',
         '}',
         'boolean shaped = token.length() == 32;',
         'boolean fresh = shaped && !expired(token);',
         'return fresh;'],
        [3, 0, 5, 1, 4, 2],
        ['The null guard has to return before anything calls length().',
         'fresh depends on shaped.',
         'The return of the result is the last line.'],
        'An early return for null is what makes every line below it able to assume the token exists.'),

    dragDrop('q_dup_if_06', 'DUPLICATE_IF_ELSE_CONDITION', 'conditional_logic', 'Advanced',
        ['if (size > 100) {',
         '    tier = "large";',
         '} else if (size > 50) {',
         '    tier = "medium";',
         ...
         '}'],
        [4, 0, 6, 2, 8, 1, 5, 3, 7],
        ['Four outcomes need four branches with strictly descending thresholds.',
         'Repeating a threshold makes everything below it unreachable.',
         'The catch-all else carries no condition.'],
        'A four-rung ladder is only correct if each threshold is strictly lower than the one above, or the lower branches can never be reached.'),

    dragDrop('q_switch_06', 'MISSING_BREAK_IN_SWITCH', 'switch_statements', 'Beginner',
        ['switch (choice) {',
         '    case 1:',
         '        start();',
         '        break;',
        ...
        [1, 4, 0, 3, 6, 2, 7, 5],
        ['A case label comes before the statements it runs.',
         'Each case needs its own break, or choice 1 also stops.',
         'The closing brace ends the switch.'],
        'Two complete cases: label, body, break, twice. Without the first break, choosing 1 would start and then immediately stop.'),

    dragDrop('q_switch_07', 'MISSING_BREAK_IN_SWITCH', 'switch_statements', 'Intermediate',
        ['switch (level) {',
         '    case "low":',
         '        limit = 10;',
         '        break;',
         ...
         '}'],
        [2, 0, 5, 3, 7, 1, 9, 8, 4, 6],
        ['Each case assigns and then breaks.',
         'Without a break, "low" would be overwritten by the next case.',
         'default needs no break because nothing follows it.'],
        'A missing break here silently produces the wrong limit rather than an error, which is what makes fall-through hard to notice.'),

    dragDrop('q_switch_08', 'MISSING_BREAK_IN_SWITCH', 'switch_statements', 'Advanced',
        ['switch (state) {',
         '    case "new":',
         '    case "queued":',
         '        enqueue();',
         ...
         '        reject();',
         '}'],
        [3, 1, 6, 0, 4, 9, 2, 10, 7, 5, 8],
        ['Two labels share one body, which is deliberate fall-through.',
         'The shared body comes after both labels and ends with one break.',
         'The "done" case is separate and needs its own break.'],
        'Stacked labels are the legitimate use of fall-through; the break after enqueue() is what stops it running on into archive().'),

    dragDrop('q_switch_09', 'MISSING_BREAK_IN_SWITCH', 'switch_statements', 'Advanced',
        ['switch (action) {',
         '    case "delete":',
         '        confirm();',
         '        remove();',
         ...
         '}'],
        [5, 1, 3, 0, 6, 2, 8, 4, 7],
        ['The delete case runs two statements before its break.',
         'Confirming has to happen before removing.',
         'Without the break, a delete would also run the read branch.'],
        'A case body can be several statements; the break goes after all of them, and here the order within the body matters as much as the break.'),
];
